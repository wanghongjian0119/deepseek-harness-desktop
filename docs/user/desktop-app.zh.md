...
应用在每次启动（以及运行期间每 6 小时）检查配置的源码仓库。当上游代码领先于内置版本时，对每个新的上游 SHA **只自动打开一次**应用内**更新中心**（记录在 `~/.dsh/desktop/offered-update.json`）——不使用系统通知。在窗口内可检查更新、下载并安装、查看进度；任务会重新构建后端（约几分钟），并以新版本重启。更新失败则保留当前版本。需要联网，重建期间需数 GB 空闲磁盘。

默认更新源是官方 `deepseek-ai/deepseek-harness` 的 master 分支（可用 `DSH_DESKTOP_UPDATE_REPO` 覆盖）——等同于你自己 pull 并安装该仓库的信任级别。内置的 Electron 壳本身不更新，只更新后端。可随时用窗口顶部常显菜单 **更新 → 打开更新中心** 打开。
