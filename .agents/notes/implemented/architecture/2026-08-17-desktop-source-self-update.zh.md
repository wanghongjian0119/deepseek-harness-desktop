...
- **更新作业**（`update-job.ts`）：下载上游源码包 → 用内置 Node 引导声明的 pnpm 版本（内置 Node 不在 PATH 上，所有 pnpm 调用以 `<node> <pnpm.cjs> ...` 执行）→ 在拉取的检出处执行 `pnpm install` + `pnpm run build`（子进程使用封闭环境：隔离的 `workDir` pnpm store，不继承 Electron 的 `NODE_OPTIONS`/`ELECTRON_*`；失败把该命令最后几十行输出带进错误）→ 复用当前载荷的 Node 可执行文件，经共享的 `assemblePayload` 构建器组装新载荷（以启动冒烟收尾）→ 翻转指针。旧载荷目录保留到下一次更新成功；构建失败保留当前版本，构建树留待排查。
...
- **UI**：启动时与每 6 小时检查，弹原生通知；确认后用带进度窗口的作业更新，随后后端以新载荷重启。菜单提供手动"检查更新"。Electron 壳本身不更新——只更新后端。
...
- 复现：`pnpm run build`、`pnpm --filter @deepseek-ai/dsh-desktop run build:payload`，启动应用；上游 master 有新提交时触发通知流程（或用菜单"检查更新"）。
